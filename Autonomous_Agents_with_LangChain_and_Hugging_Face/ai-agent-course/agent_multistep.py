...
model = AutoModelForSeq2SeqLM.from_pretrained(MODEL_ID).to("cpu")  # CPU for stability/consistency

def generate(prompt: str, max_new_tokens: int, do_sample: bool = False) -> str:
...
def process_topic(topic: str):
    ...
    summary = generate(p1, max_new_tokens=40)
    ...
    raw_title = generate(p2, max_new_tokens=10)
    ...
    raw_tags = generate(p3, max_new_tokens=12)
